...
type Props = { meetingIdParam: string | string[] | undefined };
...
export default function PublicMeetingAgendaByMeetingIdScreen({ meetingIdParam }: Props) {
  ...
  const load = useCallback(async () => {
    const mid = extractUuidFromRouteParam(meetingIdParam);
    if (!mid) {
      setState('empty');
    ...
    try {
      ...
      console.error('Public agenda load (short link):', e);
      setPayload(null);
      setState('error');
  ...
  const meetingNo = payload.meeting.meeting_number?.trim() || '0000';
