/*
  Journey Home (My Journey tab): one RPC for open meeting + stats + VPE flag + poll flags.

  Replaces multiple PostgREST round-trips on initial load.
*/
...
COMMENT ON FUNCTION public.get_journey_home_snapshot(uuid) IS
  'Authenticated club members: Journey Home header stats, current open meeting, VPE match, poll flags in one JSON payload.';

GRANT EXECUTE ON FUNCTION public.get_journey_home_snapshot(uuid) TO authenticated;
