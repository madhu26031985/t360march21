-- Treat NULL role_status as available (same as Manage / Book a Role). Only exclude Deleted.
...
COMMENT ON FUNCTION public.get_evaluation_corner_snapshot(uuid) IS
  'Authenticated club members: meeting + pathways + prepared-speaker/evaluator roles (excludes role_status Deleted only; NULL counts as available).';
