/* ExComm-chosen layout for the public web agenda; URL ?skin= still overrides. */

ALTER TABLE public.app_club_meeting
  ADD COLUMN IF NOT EXISTS public_agenda_skin text NOT NULL DEFAULT 'default';

ALTER TABLE public.app_club_meeting
  DROP CONSTRAINT IF EXISTS app_club_meeting_public_agenda_skin_check;

ALTER TABLE public.app_club_meeting
  ADD CONSTRAINT app_club_meeting_public_agenda_skin_check
  CHECK (public_agenda_skin IN ('default', 'minimal', 'vibrant'));
...
COMMENT ON FUNCTION public.get_public_meeting_agenda_by_club(uuid, text, uuid) IS
  'Public agenda: returns meeting+club+items, or JSON {error, message} for not_found / not_public.';
