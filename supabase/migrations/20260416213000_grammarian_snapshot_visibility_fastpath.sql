-- Grammarian first-paint fast path:
-- Inline summary visibility into the snapshot payload so the screen
-- avoids an extra round-trip during initial hydration.
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    CASE
      WHEN to_regclass('public.grammarian_meeting_summary_visibility') IS NULL THEN true
      ELSE COALESCE((
        SELECT v.summary_visible_to_members
        FROM public.grammarian_meeting_summary_visibility v
...
CREATE INDEX IF NOT EXISTS idx_grammarian_summary_visibility_meeting_id
  ON public.grammarian_meeting_summary_visibility(meeting_id);
