-- Track speech/category role assignments made from Timer Report vs self-book / admin.
ALTER TABLE public.app_meeting_roles_management
ADD COLUMN IF NOT EXISTS timer_report_assigned_by uuid NULL REFERENCES public.app_user_profiles (id) ON DELETE SET NULL;

COMMENT ON COLUMN public.app_meeting_roles_management.timer_report_assigned_by IS
  'User who assigned this slot from Timer Report (Timer or VPE). NULL = self-book, admin, or legacy.';
      ...
      (
        SELECT jsonb_agg(
          jsonb_build_object(
            ...
            'timer_report_assigned_by', rm.timer_report_assigned_by,
            'app_user_profiles',
            CASE
