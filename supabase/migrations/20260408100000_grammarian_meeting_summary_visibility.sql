-- Per-meeting toggle: show Grammarian Summary to members (Timer / Ah Counter pattern).
-- When false, get_grammarian_corner_snapshot omits lexicon + live observation flags for non-privileged users.

CREATE TABLE IF NOT EXISTS public.grammarian_meeting_summary_visibility (
...
ALTER TABLE public.grammarian_meeting_summary_visibility ENABLE ROW LEVEL SECURITY;

CREATE POLICY "grammarian_meeting_summary_visibility_select_club"
  ON public.grammarian_meeting_summary_visibility
  FOR SELECT
  TO authenticated
  USING (
    EXISTS (
      ...
      WHERE m.id = grammarian_meeting_summary_visibility.meeting_id
    )
  );

CREATE POLICY "grammarian_meeting_summary_visibility_insert_assigned_or_vpe"
  ON public.grammarian_meeting_summary_visibility
  FOR INSERT
  TO authenticated
  WITH CHECK (
    EXISTS (
      ...
      WHERE rm.meeting_id = grammarian_meeting_summary_visibility.meeting_id
...
CREATE POLICY "grammarian_meeting_summary_visibility_update_assigned_or_vpe"
  ON public.grammarian_meeting_summary_visibility
  FOR UPDATE
  TO authenticated
  USING (
...
COMMENT ON TABLE public.grammarian_meeting_summary_visibility IS
  'When summary_visible_to_members is false, Grammarian Summary content is hidden from general members (Grammarian + VPE still see full snapshot).';

GRANT SELECT, INSERT, UPDATE ON public.grammarian_meeting_summary_visibility TO authenticated;
...
BEGIN
  ...
  v_privileged :=
    (v_assigned_grammarian_id IS NOT NULL AND v_assigned_grammarian_id = auth.uid())
    OR EXISTS (
      SELECT 1
  ...
  SELECT COALESCE(
    (
      SELECT g.summary_visible_to_members
      FROM public.grammarian_meeting_summary_visibility g
      WHERE g.meeting_id = p_meeting_id
    ),
    true
  ) INTO v_summary_visible;

  IF NOT v_privileged AND NOT v_summary_visible THEN
    RETURN jsonb_build_object(
      ...
      'has_published_live_observations', false
    );
  END IF;
...
COMMENT ON FUNCTION public.get_grammarian_corner_snapshot(uuid) IS
  'Club members: grammarian corner snapshot; lexicon/live summary omitted when summary is hidden from members (unless Grammarian or VPE).';
