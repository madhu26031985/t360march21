/*
  # Update Agenda Generation for Ancillary Speakers
  
  ## Summary
  Updates the auto_generate_meeting_agenda function to properly handle
  the new Ancillary Speakers section by mapping it to the correct role
  classifications (timer, ah_counter, grammarian, quiz_master).
  
  ## Changes
  1. Add ancillary_speakers role classification mapping in the function
  2. Map it to: timer, ah_counter, grammarian, quiz_master roles
      ...
      FOR v_role IN
        ...
        WHERE rm.meeting_id = p_meeting_id
          ...
          AND (
            ...
            (v_template.role_classification = 'ancillary_speakers' AND rm.role_classification IN ('timer', 'ah_counter', 'grammarian', 'quiz_master')) OR
...
COMMENT ON FUNCTION auto_generate_meeting_agenda IS 
  'Auto-generates meeting agenda items based on templates and booked roles. Includes Ancillary Speakers section for Tag Team and Listener roles.';