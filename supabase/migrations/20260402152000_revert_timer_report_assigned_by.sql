-- Revert: timer_report_assigned_by on app_meeting_roles_management
-- Production hotfix:
-- - Remove the extra FK that causes PostgREST embed ambiguity (HTTP 300).
-- - Update RPCs to stop referencing the removed column.

-- 1) Drop column (and FK) safely
ALTER TABLE public.app_meeting_roles_management
  DROP COLUMN IF EXISTS timer_report_assigned_by;

-- 2) Rewrite RPC without timer_report_assigned_by
