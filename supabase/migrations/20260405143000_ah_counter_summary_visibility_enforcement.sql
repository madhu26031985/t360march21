-- Enforce ah_counter_corner_visibility for members reading Ah Counter report data.
-- When summary_visible_to_members is false, only assigned Ah Counter + VPE may see
-- full meeting rows (RPC + table SELECT). Speakers may still SELECT their own rows
-- via the separate "as speaker" policy.

-- ---------------------------------------------------------------------------
-- 1) Tighten club-member read policy on ah_counter_reports
-- ---------------------------------------------------------------------------
DROP POLICY IF EXISTS "ah_counter_reports_read_policy" ON public.ah_counter_reports;

CREATE POLICY "ah_counter_reports_read_policy"
  ON public.ah_counter_reports
  FOR SELECT
  TO authenticated
  USING (
    club_id IN (
    ...
    AND (
      EXISTS (
        ...
        WHERE rm.meeting_id = meeting_id
...
COMMENT ON POLICY "ah_counter_reports_read_policy" ON public.ah_counter_reports IS
  'Club members may read Ah Counter rows for a meeting if summary is visible to members, or if they are the assigned Ah Counter or club VPE.';

-- ---------------------------------------------------------------------------
-- 2) SECURITY DEFINER RPC must mirror the same rule (RLS is bypassed inside definer)
-- ---------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.get_ah_counter_report_rows(uuid) IS
  'Club members: Ah Counter rows when summary is visible, or always for assigned Ah Counter / VPE.';
