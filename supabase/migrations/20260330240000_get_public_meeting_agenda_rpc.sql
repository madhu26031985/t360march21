/*
  Public shareable meeting agenda (no login): one JSON payload when slug + meeting number + id match
  and is_agenda_visible is true.
*/

CREATE OR REPLACE FUNCTION public.get_public_meeting_agenda(
  p_meeting_id uuid,
  p_club_slug text,
  p_meeting_no text
)
...
BEGIN
  ...
  WHERE cp.club_id = v_meeting.club_id
  LIMIT 1;

  IF v_club_name IS NULL OR btrim(v_club_name) = '' THEN
    SELECT c.name INTO v_club_name FROM public.clubs c WHERE c.id = v_meeting.club_id LIMIT 1;
  END IF;
  ...
  v_expected_slug := lower(regexp_replace(v_club_name, '[^a-zA-Z0-9]', '', 'g'));
  IF v_expected_slug IS NULL OR v_expected_slug = '' THEN
    v_expected_slug := 'club';
  END IF;
  ...
  v_slug_param := lower(regexp_replace(COALESCE(trim(p_club_slug), ''), '[^a-z0-9]', '', 'g'));
  v_num_param := regexp_replace(COALESCE(trim(p_meeting_no), ''), '[^a-zA-Z0-9._-]', '', 'g');
  IF v_num_param IS NULL OR v_num_param = '' THEN
    v_num_param := '0';
  END IF;

  IF v_slug_param IS DISTINCT FROM v_expected_slug THEN
    RETURN NULL;
  END IF;
...
COMMENT ON FUNCTION public.get_public_meeting_agenda(uuid, text, text) IS
  'Public agenda HTML/share: meeting + visible items + names; requires slug and meeting number to match; respects is_agenda_visible.';

GRANT EXECUTE ON FUNCTION public.get_public_meeting_agenda(uuid, text, text) TO anon;
GRANT EXECUTE ON FUNCTION public.get_public_meeting_agenda(uuid, text, text) TO authenticated;
