-- Grammarian Corner: reduce pre-meeting startup calls by bundling
-- daily elements + word/idiom/quote + published-live flag in snapshot.
...
COMMENT ON FUNCTION public.get_grammarian_corner_snapshot(uuid) IS
  'Authenticated club members: grammarian corner snapshot (header + pre-meeting lexicon + live-published flag).';

GRANT EXECUTE ON FUNCTION public.get_grammarian_corner_snapshot(uuid) TO authenticated;

CREATE INDEX IF NOT EXISTS idx_grammarian_daily_elements_meeting_user_created
  ON public.app_grammarian_daily_elements (meeting_id, grammarian_user_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_grammarian_idiom_meeting_user
  ON public.grammarian_idiom_of_the_day (meeting_id, grammarian_user_id);

CREATE INDEX IF NOT EXISTS idx_grammarian_quote_meeting_user
  ON public.grammarian_quote_of_the_day (meeting_id, grammarian_user_id);
