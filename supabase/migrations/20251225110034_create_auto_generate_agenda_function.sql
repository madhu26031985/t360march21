/*
  # Create Auto-Generate Agenda Function

  ## Summary
  Creates a PostgreSQL function to automatically generate meeting agenda items
  based on templates and booked roles. This function pulls data from:
  - Agenda templates
  - Meeting role bookings
  - User assignments
  - Speech details (title, pathway, project)

  ## Function
  - `auto_generate_meeting_agenda(meeting_id UUID)` - Generates complete agenda for a meeting

  ## Logic
  1. Clear existing auto-generated items
  2. Get meeting details (start time, theme, etc.)
  3. For each template section:
     - If role-based, get all matching booked roles
     - Create agenda item with role details
     - Calculate timings
  4. Insert all items with proper ordering
*/
...
COMMENT ON FUNCTION auto_generate_meeting_agenda IS 
  'Auto-generates meeting agenda items based on templates and booked roles. Called when a meeting is created or roles are updated.';
