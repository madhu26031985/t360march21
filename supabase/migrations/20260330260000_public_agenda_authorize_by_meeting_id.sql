/*
  Public agenda: authorize by meeting id + is_agenda_visible only.
  URL path still includes club id and meeting number for readable sharing, but typos there
  no longer block loading (meeting UUID is the secret).
*/
    ...
    'club', COALESCE(
      (
        SELECT jsonb_build_object(
          'club_name',
            COALESCE(
              NULLIF(trim(BOTH FROM COALESCE(cp.club_name, c.name, '')), ''),
              'Club'
            ),
...
COMMENT ON FUNCTION public.get_public_meeting_agenda_by_club(uuid, text, uuid) IS
  'Public agenda: p_meeting_id + is_agenda_visible; p_club_id and p_meeting_no ignored (readable URL only).';
