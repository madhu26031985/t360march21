...
import { ArrowLeft, Book, Calendar, Clock, MapPin, ChevronRight, ExternalLink, Building2, Crown, User, Shield, Eye, UserCheck } from 'lucide-react-native';
...
interface MeetingWithMinutes extends Meeting {
...
export default function MeetingMinutes() {
  ...
  const [meetings, setMeetings] = useState<MeetingWithMinutes[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [selectedTab, setSelectedTab] = useState<'open' | 'closed'>('open');
  const [filteredMeetings, setFilteredMeetings] = useState<MeetingWithMinutes[]>([]);
  ...
  const loadMeetings = async () => {
    ...
    try {
      ...
      // Combine meetings with their minutes
      const meetingsWithMinutes = meetingsData.map(meeting => ({
        ...meeting,
        minutes: minutesData?.find(minutes => minutes.meeting_id === meeting.id)
      }));
  ...
  const handleMeetingPress = (meeting: Meeting) => {
    const meetingWithMinutes = meeting as MeetingWithMinutes;
    if (meetingWithMinutes.minutes) {
      handleViewMinutes(meetingWithMinutes.minutes);
    } else {
      Alert.alert('No Minutes', 'No minutes have been uploaded for this meeting yet.');
    }
  };

  const handleViewMinutes = async (minutes: MeetingMinutes) => {
    try {
      if (minutes.document_type === 'pdf_file') {
        Alert.alert('PDF File', 'PDF file viewing is not yet supported. Please contact your ExComm for assistance.');
      } else if (minutes.document_url) {
        if (WebBrowser && WebBrowser.openBrowserAsync) {
          await WebBrowser.openBrowserAsync(minutes.document_url);
        } else {
          await Linking.openURL(minutes.document_url);
        }
      } else {
        Alert.alert('Error', 'No minutes URL available');
      }
    } catch (error) {
      console.error('Error opening minutes:', error);
      Alert.alert('Error', 'Failed to open minutes');
    }
  };
  ...
  const MeetingCard = ({ meeting }: { meeting: MeetingWithMinutes }) => (
    <TouchableOpacity 
    ...
    >
      <View style={styles.meetingHeader}>
        ...
        <View style={styles.meetingCardInfo}>
          ...
          {/* Minutes Status */}
          <View style={styles.minutesStatus}>
            {meeting.minutes ? (
              <View style={[styles.minutesAvailable, { backgroundColor: '#10b981' + '20' }]}>
                <Book size={12} color="#10b981" />
                <Text style={[styles.minutesStatusText, { color: '#10b981' }]} maxFontSizeMultiplier={1.3}>
                  Minutes Available
                </Text>
              </View>
            ) : (
              <View style={[styles.minutesNotAvailable, { backgroundColor: theme.colors.textSecondary + '20' }]}>
                <Book size={12} color={theme.colors.textSecondary} />
                <Text style={[styles.minutesStatusText, { color: theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>
                  No Minutes
                </Text>
              </View>
            )}
        ...
        <View style={styles.meetingCardActions}>
          {meeting.minutes ? (
            <View style={[styles.viewMinutesButton, { backgroundColor: '#10b981' }]}>
              <ExternalLink size={14} color="#ffffff" />
            </View>
          ) : (
            <View style={[styles.noMinutesButton, { backgroundColor: theme.colors.textSecondary + '20' }]}>
              <Book size={14} color={theme.colors.textSecondary} />
            </View>
          )}
        </View>
      </View>
    ...
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
        <View style={styles.loadingContainer}>
          <Text style={[styles.loadingText, { color: theme.colors.text }]} maxFontSizeMultiplier={1.3}>Loading meeting minutes...</Text>
        </View>
      </SafeAreaView>
    );
  }
    ...
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
        <View style={styles.accessDeniedContainer}>
          <Book size={48} color={theme.colors.textSecondary} />
          ...
          <Text style={[styles.accessDeniedMessage, { color: theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>
            You need to be an authenticated club member to access meeting minutes.
          </Text>
          <TouchableOpacity
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: theme.colors.surface, borderBottomColor: theme.colors.border }]}>
        ...
        <Text style={[styles.headerTitle, { color: theme.colors.text }]} maxFontSizeMultiplier={1.3}>Meeting Minutes</Text>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.infoCard, { backgroundColor: theme.colors.surface }]}>
          <View style={styles.infoHeader}>
            ...
            <Text style={[styles.infoTitle, { color: theme.colors.text }]} maxFontSizeMultiplier={1.3}>Meeting Minutes</Text>
          </View>
          <Text style={[styles.infoDescription, { color: theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>
            View and access minutes for club meetings. Minutes are uploaded by your Executive Committee.
          </Text>
        </View>
        ...
        <View style={styles.meetingsSection}>
          ...
          <Text style={[styles.sectionSubtitle, { color: theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>
            {selectedTab === 'open' 
              ? 'View minutes for upcoming meetings'
              : 'Access minutes from past meetings'
            }
          </Text>
