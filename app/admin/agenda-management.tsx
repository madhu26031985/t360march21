import { View, Text, StyleSheet, ScrollView, TouchableOpacity, Modal, TextInput, Alert, KeyboardAvoidingView } from 'react-native';
...
export default function AgendaManagement() {
  ...
  const [meetingAgendas, setMeetingAgendas] = useState<{ [meetingId: string]: MeetingAgenda }>({});
  const [showUploadModal, setShowUploadModal] = useState(false);
  const [selectedMeeting, setSelectedMeeting] = useState<Meeting | null>(null);
  const [existingAgenda, setExistingAgenda] = useState<MeetingAgenda | null>(null);
  ...
  useEffect(() => {
    // Only filter when agendas are loaded
    if (!isLoadingAgendas) {
      filterMeetings();
    }
  }, [meetings, selectedTab, searchQuery, dateFilter, customStartDate, customEndDate, meetingAgendas, isLoadingAgendas]);
  ...
  const loadMeetingAgendas = async () => {
    ...
    try {
      ...
      if (error) {
        console.error('Error loading meeting agendas:', error);
        return;
      }

      // Create a map of meeting_id to agenda
      const agendasMap: { [meetingId: string]: MeetingAgenda } = {};
      (data || []).forEach(agenda => {
        agendasMap[agenda.meeting_id] = agenda;
      });
      
      setMeetingAgendas(agendasMap);
    } catch (error) {
      console.error('Error loading meeting agendas:', error);
    }
  };
  ...
  const filterMeetings = () => {
    console.log('Filtering meetings:', {
      ...
      agendasLoaded: Object.keys(meetingAgendas).length,
      selectedTab,
    ...
    // Apply agenda status filter
    if (selectedTab === 'pending') {
      // Show meetings without agendas (pending agenda upload)
      const pendingMeetings = filtered.filter(m => !meetingAgendas[m.id]);
      console.log('Pending meetings:', pendingMeetings.length);
      setFilteredMeetings(pendingMeetings);
    } else {
      // Show meetings with agendas (agenda available)
      const availableMeetings = filtered.filter(m => meetingAgendas[m.id]);
      console.log('Available meetings:', availableMeetings.length);
      setFilteredMeetings(availableMeetings);
    }
  };

  const handleUploadAgenda = async (meeting: Meeting) => {
    ...
    try {
      ...
      if (error && error.code !== 'PGRST116') {
        console.error('Error checking existing agenda:', error);
      }

      if (data) {
        setExistingAgenda(data);
        setAgendaForm({
          title: data.title,
        ...
        setAgendaForm({
          title: `Meeting Agenda - ${new Date(meeting.meeting_date).toLocaleDateString()}`,
          document_type: 'pdf_url',
          document_url: '',
          pdf_data: null,
        });
      ...
      console.error('Error loading existing agenda:', error);
    }
      ...
      if (!result.canceled && result.assets[0]) {
        ...
        reader.onload = () => {
          ...
          setAgendaForm(prev => ({
            ...prev,
            pdf_data: base64Data,
            document_url: asset.name || 'agenda.pdf'
          }));
        };
  ...
  const validateForm = (): boolean => {
    if (!agendaForm.title.trim()) {
      Alert.alert('Error', 'Please enter an agenda title');
      return false;
    }

    if (agendaForm.document_type === 'pdf_url' || agendaForm.document_type === 'website_url') {
      if (!agendaForm.document_url.trim()) {
        Alert.alert('Error', 'Please enter a document URL');
        return false;
      }
      
      try {
        new URL(agendaForm.document_url.trim());
      } catch {
    ...
    if (agendaForm.document_type === 'pdf_file' && !agendaForm.pdf_data) {
  ...
  const handleSaveAgenda = async () => {
    ...
    try {
      const saveData = {
        ...
        document_type: agendaForm.document_type,
        document_url: (agendaForm.document_type === 'pdf_url' || agendaForm.document_type === 'website_url') 
          ? agendaForm.document_url.trim() 
          : null,
        pdf_data: agendaForm.document_type === 'pdf_file' ? agendaForm.pdf_data : null,
      };
      ...
      if (existingAgenda) {
        ...
        if (error) {
          ...
          Alert.alert('Error', 'Failed to update agenda');
          return;
        }

        Alert.alert('Success', 'Meeting agenda updated successfully');
      } else {
        // Create new agenda
        const { error } = await supabase
          .from('meeting_agendas')
        ...
        if (error) {
          ...
          Alert.alert('Error', 'Failed to upload agenda');
          return;
        }

        Alert.alert('Success', 'Meeting agenda uploaded successfully');
      }
      ...
      // Reload agendas to update the filtering
      loadMeetingAgendas();
    } catch (error) {
  ...
  const updateFormField = (field: keyof AgendaForm, value: any) => {
  ...
  const MeetingCard = ({ meeting, hasAgenda }: { meeting: Meeting; hasAgenda: boolean }) => (
      ...
      <View style={styles.meetingHeader}>
        <View style={styles.meetingInfo}>
          ...
          <View style={styles.meetingMeta}>
            ...
            <View style={[
              styles.statusTag,
              { backgroundColor: hasAgenda ? '#10b981' + '20' : '#f59e0b' + '20' }
            ]}>
              <Text style={[
                styles.statusText,
                { color: hasAgenda ? '#10b981' : '#f59e0b' }
              ]} maxFontSizeMultiplier={1.3}>
                {hasAgenda ? 'Available' : 'Pending'}
              </Text>
            </View>
          </View>
        ...
        <View style={styles.meetingActions}>
          <TouchableOpacity
            style={[styles.actionButton, { backgroundColor: hasAgenda ? '#f0fdf4' : '#fef3c7' }]}
            onPress={() => handleUploadAgenda(meeting)}
            activeOpacity={0.7}
          >
            <Upload size={14} color={hasAgenda ? '#10b981' : '#f59e0b'} />
          </TouchableOpacity>
        </View>
      </View>
    </View>
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: theme.colors.surface, borderBottomColor: theme.colors.border }]}>
        ...
        <Text style={[styles.headerTitle, { color: theme.colors.text }]} maxFontSizeMultiplier={1.3}>Agenda Management</Text>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          ...
          >
            ...
            <View style={[
              ...
              <Text style={[
                ...
                {isLoadingAgendas ? '...' : meetings.filter(m => !meetingAgendas[m.id]).length}
              </Text>
            </View>
          </TouchableOpacity>
        ...
        <View style={styles.meetingsSection}>
          {isLoadingAgendas ? (
            <View style={styles.loadingContainer}>
              <Text style={[styles.loadingText, { color: theme.colors.text }]} maxFontSizeMultiplier={1.3}>Loading agenda data...</Text>
            </View>
          ) : (
            <>
          <Text style={[styles.sectionTitle, { color: theme.colors.text }]} maxFontSizeMultiplier={1.3}>
            {selectedTab === 'pending' ? 'Meetings Pending Agenda' : 'Meetings with Agenda Available'} ({filteredMeetings.length})
          </Text>
          
          {filteredMeetings.map((meeting) => (
            <MeetingCard key={meeting.id} meeting={meeting} hasAgenda={!!meetingAgendas[meeting.id]} />
          ))}
          ...
          {filteredMeetings.length === 0 && (
            <View style={styles.emptyState}>
              ...
              <Text style={[styles.emptyStateText, { color: theme.colors.text }]} maxFontSizeMultiplier={1.3}>
                No meetings {selectedTab === 'pending' ? 'pending agenda upload' : 'with agendas available'}
              </Text>
              <Text style={[styles.emptyStateSubtext, { color: theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>
                {selectedTab === 'pending' 
                  ? 'Meetings without agendas will appear here'
                  : 'Meetings with uploaded agendas will appear here'
                }
              </Text>
            </View>
          )}
          ...
          <View style={[styles.centerUploadModal, { backgroundColor: theme.colors.background }]}>
            ...
            <View style={[styles.modalHeader, { backgroundColor: theme.colors.surface, borderBottomColor: theme.colors.border }]}>
              <Text style={[styles.modalTitle, { color: theme.colors.text }]} maxFontSizeMultiplier={1.3}>
                {existingAgenda ? 'Update Meeting Agenda' : 'Upload Meeting Agenda'}
              </Text>
            ...
            <ScrollView style={styles.modalContent} showsVerticalScrollIndicator={false}>
              ...
              <View style={styles.formField}>
                <Text style={[styles.fieldLabel, { color: theme.colors.text }]} maxFontSizeMultiplier={1.3}>Agenda Title *</Text>
                <TextInput
                  style={[styles.textInput, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border, color: theme.colors.text }]}
                  placeholder="e.g., Meeting Agenda - January 15, 2024"
                  placeholderTextColor={theme.colors.textSecondary}
              ...
              <View style={styles.formField}>
                ...
                <TouchableOpacity
                  style={[
                    ...
                    {
                      backgroundColor: agendaForm.document_type === 'pdf_url' ? '#fef2f2' : theme.colors.surface,
                      borderColor: agendaForm.document_type === 'pdf_url' ? '#ef4444' : theme.colors.border,
                    }
                  ]}
                  onPress={() => updateFormField('document_type', 'pdf_url')}
                >
                  <View style={[styles.radioButton, { borderColor: agendaForm.document_type === 'pdf_url' ? '#ef4444' : theme.colors.border }]}>
                    {agendaForm.document_type === 'pdf_url' && (
                ...
                <TouchableOpacity
                  style={[
                    styles.documentTypeOption,
                    {
                      backgroundColor: agendaForm.document_type === 'pdf_file' ? '#f0fdf4' : theme.colors.surface,
                      borderColor: agendaForm.document_type === 'pdf_file' ? '#10b981' : theme.colors.border,
                    }
                  ]}
                  onPress={() => updateFormField('document_type', 'pdf_file')}
                >
                  <View style={[styles.radioButton, { borderColor: agendaForm.document_type === 'pdf_file' ? '#10b981' : theme.colors.border }]}>
                    {agendaForm.document_type === 'pdf_file' && (
                ...
                <TouchableOpacity
                  style={[
                    styles.documentTypeOption,
                    {
                      backgroundColor: agendaForm.document_type === 'website_url' ? '#f3e8ff' : theme.colors.surface,
                      borderColor: agendaForm.document_type === 'website_url' ? '#8b5cf6' : theme.colors.border,
                    }
                  ]}
                  onPress={() => updateFormField('document_type', 'website_url')}
                >
                  <View style={[styles.radioButton, { borderColor: agendaForm.document_type === 'website_url' ? '#8b5cf6' : theme.colors.border }]}>
                    {agendaForm.document_type === 'website_url' && (
              ...
              {(agendaForm.document_type === 'pdf_url' || agendaForm.document_type === 'website_url') && (
                <View style={styles.formField}>
                  <Text style={[styles.fieldLabel, { color: theme.colors.text }]} maxFontSizeMultiplier={1.3}>
                    {agendaForm.document_type === 'pdf_url' ? 'PDF URL *' : 'Website Link *'}
                  </Text>
                  <TextInput
                    style={[styles.textInput, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border, color: theme.colors.text }]}
                    placeholder="https://example.com/agenda.pdf"
                    placeholderTextColor={theme.colors.textSecondary}
                    value={agendaForm.document_url}
                ...
                <View style={styles.formField}>
                  ...
                  >
                    ...
                    <Text style={[styles.filePickerText, { color: theme.colors.text }]} maxFontSizeMultiplier={1.3}>
                      {agendaForm.pdf_data ? 'PDF Selected' : 'Select PDF File'}
                    </Text>
                  </TouchableOpacity>
                  {agendaForm.pdf_data && (
              ...
              <View style={styles.modalActions}>
                ...
                >
                  ...
                  <Text style={styles.uploadButtonText} maxFontSizeMultiplier={1.3}>
                    {isSaving ? 'Uploading...' : (existingAgenda ? 'Update Agenda' : 'Upload Agenda')}
                  </Text>
                </TouchableOpacity>
              </View>
            </ScrollView>
