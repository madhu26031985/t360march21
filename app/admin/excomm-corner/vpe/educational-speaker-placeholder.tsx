...
interface EducationalSpeakerData {
  id: string;
  meeting_id: string;
...
export default function EducationalSpeakerPlaceholder() {
  ...
  const [speakerData, setSpeakerData] = useState<EducationalSpeakerData | null>(null);
  ...
  const handleMeetingSelect = async (meeting: Meeting) => {
    ...
    try {
      const { data: existingData, error: dataError } = await supabase
        ...
        .select('*, app_user_profiles(id, full_name)')
      ...
      if (existingData) {
        const assignedUserProfile = existingData.app_user_profiles as any;
        setAssignedUser({
          id: existingData.speaker_user_id,
        ...
        const { data: eduRoleData, error: roleError } = await supabase
          .from('app_meeting_roles_management')
          .select('assigned_user_id, app_user_profiles(id, full_name)')
          .eq('meeting_id', meeting.id)
          .ilike('role_name', '%educational%')
          .eq('booking_status', 'booked')
          .maybeSingle();

        if (roleError && roleError.code !== 'PGRST116') throw roleError;

        if (eduRoleData?.assigned_user_id) {
          const assignedUserProfile = eduRoleData.app_user_profiles as any;
          setAssignedUser({
            id: eduRoleData.assigned_user_id,
            full_name: assignedUserProfile?.full_name || 'Unknown',
          });
        }
      }
  ...
  const handleSave = async () => {
    if (!selectedMeeting || !user?.id || !user?.currentClubId || !assignedUser) return;

    if (!speechTitle.trim()) {
      Alert.alert('Error', 'Educational speech title is required');
      return;
    }
    ...
    try {
      ...
      if (speakerData) {
        ...
        const { error: insertError } = await supabase
          ...
          .insert({
            ...
            speaker_user_id: assignedUser.id,
            speech_title: speechTitle.trim(),
            summary: summary.trim() || null,
          });
      ...
      Alert.alert('Success', 'Educational speech data saved successfully');
      await handleMeetingSelect(selectedMeeting);
    } catch (error: any) {
      console.error('Error saving educational speaker data:', error);
      Alert.alert('Error', error.message || 'Failed to save educational speech data');
    } finally {
      setSaving(false);
    }
  };
      ...
      return (
        <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
          ...
          <View style={styles.centerContainer}>
            ...
            <Text style={[styles.emptyTitle, { color: theme.colors.text }]}>Educational Speaker is not booked.</Text>
            <Text style={[styles.emptyMessage, { color: theme.colors.textSecondary }]}>
              No one has been assigned as Educational Speaker for Meeting #{selectedMeeting.meeting_number}.
            </Text>
          </View>
        </SafeAreaView>
      );
        ...
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={[styles.infoCard, { backgroundColor: '#3b82f6' + '10', borderColor: '#3b82f6' + '30' }]}>
            <Text style={[styles.infoText, { color: theme.colors.text }]} maxFontSizeMultiplier={1.3}>
              This is a backdoor entry for VPE to manage educational speaker data if the speaker forgets to enter it.
            </Text>
          </View>

          <View style={[styles.formCard, { backgroundColor: theme.colors.surface }]}>
            <Text style={[styles.formTitle, { color: theme.colors.text }]} maxFontSizeMultiplier={1.3}>
              Educational Speech Details
            </Text>
            ...
            <View style={styles.inputGroup}>
              ...
              <TextInput
                ...
                placeholder="Enter educational speech title"
            ...
            <View style={styles.inputGroup}>
              ...
              <TextInput
                ...
                placeholder="Include key learning points, practical applications, and how members can benefit (optional)"
                placeholderTextColor={theme.colors.textSecondary}
                value={summary}
