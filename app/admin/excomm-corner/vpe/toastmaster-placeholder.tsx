...
export default function ToastmasterPlaceholder() {
  ...
  const [toastmasterData, setToastmasterData] = useState<ToastmasterData | null>(null);
  ...
  const handleMeetingSelect = async (meeting: Meeting) => {
    ...
    setToastmasterData(null);
    setThemeOfDay('');
    setThemeSummary('');

    try {
      const { data: toastmasterRoleData, error: roleError } = await supabase
      ...
      const assignedToastmasterId = toastmasterRoleData?.assigned_user_id || null;

      if (assignedToastmasterId) {
        const assignedUserProfile = toastmasterRoleData.app_user_profiles as any;
        setAssignedUser({
          id: assignedToastmasterId,
          full_name: assignedUserProfile?.full_name || 'Unknown',
        });
      }

      const { data: allThemeData, error: themeError } = await supabase
      ...
      if (allThemeData && allThemeData.length > 0) {
        const matchedEntry = assignedToastmasterId
          ? allThemeData.find((e: any) => e.toastmaster_user_id === assignedToastmasterId) || allThemeData[0]
          : allThemeData[0];
        setToastmasterData(matchedEntry);
        setThemeOfDay(matchedEntry.theme_of_the_day || '');
        setThemeSummary(matchedEntry.theme_summary || '');
      }
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    if (!themeOfDay.trim()) {
      Alert.alert('Error', 'Theme of the day is required');
      return;
    }
    ...
    try {
      ...
      const { data: toastmasterRoleData } = await supabase
        .from('app_meeting_roles_management')
      ...
      const toastmasterUserId = toastmasterRoleData?.assigned_user_id || user.id;

      if (toastmasterData) {
        const { error: updateError } = await supabase
          .from('toastmaster_meeting_data')
          .update({
            theme_of_the_day: themeOfDay.trim(),
            theme_summary: themeSummary.trim() || null,
            updated_at: new Date().toISOString(),
          })
          .eq('id', toastmasterData.id);
        ...
        const { error: insertError } = await supabase
          ...
          .insert({
            ...
            toastmaster_user_id: toastmasterUserId,
            theme_of_the_day: themeOfDay.trim(),
            theme_summary: themeSummary.trim() || null,
          });
      ...
      Alert.alert('Success', 'Theme data saved successfully');
      await handleMeetingSelect(selectedMeeting);
    } catch (error: any) {
      console.error('Error saving theme data:', error);
      Alert.alert('Error', error.message || 'Failed to save theme data');
    } finally {
      setSaving(false);
    }
  };
      ...
      return (
        <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
          ...
          <View style={styles.centerContainer}>
            ...
            <Text style={[styles.emptyTitle, { color: theme.colors.text }]}>Toastmaster of the Day is not booked.</Text>
            <Text style={[styles.emptyMessage, { color: theme.colors.textSecondary }]}>
              No one has been assigned as Toastmaster for Meeting #{selectedMeeting.meeting_number}.
            </Text>
          </View>
        </SafeAreaView>
      );
        ...
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={[styles.infoCard, { backgroundColor: '#3b82f6' + '10', borderColor: '#3b82f6' + '30' }]}>
            <Text style={[styles.infoText, { color: theme.colors.text }]} maxFontSizeMultiplier={1.3}>
              This is a backdoor entry for VPE to manage theme data if the Toastmaster forgets to enter it.
            </Text>
          </View>
          ...
          <View style={[styles.formCard, { backgroundColor: theme.colors.surface }]}>
            ...
            <View style={styles.inputGroup}>
              <Text style={[styles.inputLabel, { color: theme.colors.text }]} maxFontSizeMultiplier={1.3}>
                Theme <Text style={{ color: theme.colors.error }}>*</Text>
              </Text>
              <TextInput
                style={[styles.input, {
                ...
                placeholder="Enter theme of the day"
                placeholderTextColor={theme.colors.textSecondary}
                value={themeOfDay}
                onChangeText={(text) => text.length <= 200 && setThemeOfDay(text)}
                multiline
                numberOfLines={2}
                maxLength={200}
              />
              <Text style={[styles.charCount, { color: themeOfDay.length >= 200 ? theme.colors.error : theme.colors.textSecondary }]}>
                {themeOfDay.length}/200
              </Text>
            </View>
            ...
            <View style={styles.inputGroup}>
              ...
              <TextInput
                ...
                placeholderTextColor={theme.colors.textSecondary}
                value={themeSummary}
                onChangeText={(text) => text.length <= 600 && setThemeSummary(text)}
                multiline
                numberOfLines={6}
                textAlignVertical="top"
                maxLength={600}
              />
              <Text style={[styles.charCount, { color: themeSummary.length >= 600 ? theme.colors.error : theme.colors.textSecondary }]}>
                {themeSummary.length}/600
              </Text>
            </View>
          </View>
