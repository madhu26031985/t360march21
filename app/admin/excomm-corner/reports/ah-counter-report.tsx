...
type MeetingData = {
  ...
  meeting_date: string;
  ah_counter_name: string | null;
};

export default function AhCounterReport() {
  ...
  const loadMeetings = async () => {
    ...
    try {
      ...
      const formattedMeetings: MeetingData[] = (data || []).map((meeting: any) => {
        let ahCounterName = null;

        if (Array.isArray(meeting.app_meeting_roles_management)) {
          const ahCounterRole = meeting.app_meeting_roles_management.find(
            (role: any) => role.role_name === 'Ah-Counter'
          );

          if (ahCounterRole && ahCounterRole.app_user_profiles) {
            ahCounterName = ahCounterRole.app_user_profiles.full_name;
          }
        }
                  ...
                  <View style={styles.detailRow}>
                    ...
                    <View style={styles.detailContent}>
                      ...
                      <Text style={[styles.detailValue, { color: theme.colors.text }]} maxFontSizeMultiplier={1.3}>
                        {meeting.ah_counter_name || 'Not assigned'}
                      </Text>
                    </View>
                  </View>
                </View>
