...
import { ArrowLeft, Save, StickyNote, BookOpen, ThumbsUp, AlertCircle, MessageSquareQuote, Lightbulb, Plus, X, CheckCircle2, AlertTriangle, Clock, Trash2, TrendingUp, Minus, MinusCircle, User, Eye, EyeOff, BarChart2 } from 'lucide-react-native';
...
export default function GrammarianNotes() {
  ...
  const [activeTab, setActiveTab] = useState<'notes' | 'pre-meeting' | 'live-meeting' | 'word' | 'idiom' | 'quote'>('notes');
  const [preMeetingSubTab, setPreMeetingSubTab] = useState<'word' | 'idiom' | 'quote'>('word');
  ...
  const [hasUnsavedWordChanges, setHasUnsavedWordChanges] = useState(false);
  ...
  const [hasUnsavedIdiomChanges, setHasUnsavedIdiomChanges] = useState(false);
  ...
  const handleWordChange = (text: string) => {
    if (text.length <= 500) {
      setWord(text);
      setHasUnsavedWordChanges(
        text !== (wordOfTheDay?.word || '') ||
        partOfSpeech !== (wordOfTheDay?.part_of_speech || '') ||
        meaning !== (wordOfTheDay?.meaning || '') ||
        usage !== (wordOfTheDay?.usage || '')
      );
    }
  };

  const handlePartOfSpeechChange = (text: string) => {
    if (text.length <= 100) {
      setPartOfSpeech(text);
      setHasUnsavedWordChanges(
        word !== (wordOfTheDay?.word || '') ||
        text !== (wordOfTheDay?.part_of_speech || '') ||
        meaning !== (wordOfTheDay?.meaning || '') ||
        usage !== (wordOfTheDay?.usage || '')
      );
    }
  ...
  const handleUsageChange = (text: string) => {
    if (text.length <= 500) {
      setUsage(text);
      setHasUnsavedWordChanges(
        word !== (wordOfTheDay?.word || '') ||
  ...
  const handleSaveWord = async () => {
    ...
    try {
      if (wordOfTheDay) {
        ...
        if (!word.trim()) {
          setHasUnsavedWordChanges(false);
          return;
        }
        const { error } = await supabase
          .from('grammarian_word_of_the_day')
  ...
  const handleClearWord = async () => {
    ...
    if (wordOfTheDay) {
      await supabase
        .from('grammarian_word_of_the_day')
        .update({ word: null, part_of_speech: null, meaning: null, usage: null, is_published: false, updated_at: new Date().toISOString() })
        .eq('id', wordOfTheDay.id);
      await loadWordOfTheDayData();
    }
    setHasUnsavedWordChanges(false);
  ...
  // Idiom handlers
  const handleIdiomChange = (text: string) => {
    if (text.length <= 500) {
      setIdiom(text);
      setHasUnsavedIdiomChanges(
        text !== (idiomOfTheDay?.idiom || '') ||
        idiomMeaning !== (idiomOfTheDay?.meaning || '') ||
        idiomUsage !== (idiomOfTheDay?.usage || '')
      );
    }
  };

  const handleIdiomMeaningChange = (text: string) => {
    if (text.length <= 500) {
      setIdiomMeaning(text);
      setHasUnsavedIdiomChanges(
        idiom !== (idiomOfTheDay?.idiom || '') ||
        text !== (idiomOfTheDay?.meaning || '') ||
        idiomUsage !== (idiomOfTheDay?.usage || '')
      );
    }
  };

  const handleIdiomUsageChange = (text: string) => {
    if (text.length <= 500) {
      setIdiomUsage(text);
      setHasUnsavedIdiomChanges(
        idiom !== (idiomOfTheDay?.idiom || '') ||
        idiomMeaning !== (idiomOfTheDay?.meaning || '') ||
        text !== (idiomOfTheDay?.usage || '')
      );
    }
  };
  ...
  const handleSaveIdiom = async () => {
    ...
    try {
      if (idiomOfTheDay) {
        ...
        if (!idiom.trim()) {
          setHasUnsavedIdiomChanges(false);
          return;
        }
        const { error } = await supabase
          .from('grammarian_idiom_of_the_day')
      ...
      setHasUnsavedIdiomChanges(false);
      await loadIdiomOfTheDayData();
    } catch (error) {
      console.error('Error saving idiom of the day:', error);
      Alert.alert('Error', 'An unexpected error occurred');
    } finally {
      setIsSaving(false);
    }
  ...
  const handleClearIdiom = async () => {
    ...
    if (idiomOfTheDay) {
      await supabase
        .from('grammarian_idiom_of_the_day')
        .update({ idiom: null, meaning: null, usage: null, is_published: false, updated_at: new Date().toISOString() })
        .eq('id', idiomOfTheDay.id);
      await loadIdiomOfTheDayData();
    }
    setHasUnsavedIdiomChanges(false);
  ...
  const handleQuoteChange = (text: string) => {
    if (text.length <= 150) {
      ...
      setHasUnsavedQuoteChanges(
        ...
        quoteMeaning !== (quoteOfTheDay?.meaning || '')
      );
    }
  };

  const handleQuoteMeaningChange = (text: string) => {
    if (text.length <= 400) {
      setQuoteMeaning(text);
      setHasUnsavedQuoteChanges(
        quote !== (quoteOfTheDay?.quote || '') ||
        text !== (quoteOfTheDay?.meaning || '')
      );
    }
  };
  ...
  const handleSaveQuote = async () => {
    ...
    try {
      if (quoteOfTheDay) {
        ...
        Alert.alert('Success', 'Quote of the day saved successfully');
      } else {
        if (!quote.trim()) {
      ...
      setHasUnsavedQuoteChanges(false);
      await loadQuoteOfTheDayData();
    } catch (error) {
      console.error('Error saving quote of the day:', error);
      Alert.alert('Error', 'An unexpected error occurred');
    } finally {
      setIsSaving(false);
    }
  ...
  const handleClearQuote = async () => {
    ...
    if (quoteOfTheDay) {
      await supabase
        .from('grammarian_quote_of_the_day')
        .update({ quote: null, meaning: null, is_published: false, updated_at: new Date().toISOString() })
        .eq('id', quoteOfTheDay.id);
      await loadQuoteOfTheDayData();
    }
    setHasUnsavedQuoteChanges(false);
    ...
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
        <View style={styles.loadingContainer}>
          <Text style={[styles.loadingText, { color: theme.colors.text }]} maxFontSizeMultiplier={1.3}>Loading prep space...</Text>
        </View>
      </SafeAreaView>
    );
  }
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
        ...
        {activeTab === 'word' && (
          <TouchableOpacity
            style={[
              styles.saveButton,
              {
                backgroundColor: hasUnsavedWordChanges ? theme.colors.primary : theme.colors.surface,
                borderColor: theme.colors.border,
              }
            ]}
            onPress={handleSaveWord}
            disabled={isSaving || !hasUnsavedWordChanges}
          >
            <Save size={16} color={hasUnsavedWordChanges ? "#ffffff" : theme.colors.textSecondary} />
          </TouchableOpacity>
        )}
        {activeTab === 'idiom' && (
          <TouchableOpacity
            style={[
              styles.saveButton,
              {
                backgroundColor: hasUnsavedIdiomChanges ? theme.colors.primary : theme.colors.surface,
                borderColor: theme.colors.border,
              }
            ]}
            onPress={handleSaveIdiom}
            disabled={isSaving || !hasUnsavedIdiomChanges}
          >
            <Save size={16} color={hasUnsavedIdiomChanges ? "#ffffff" : theme.colors.textSecondary} />
          </TouchableOpacity>
        )}
        {activeTab === 'quote' && (
          <TouchableOpacity
            style={[
              styles.saveButton,
              {
                backgroundColor: hasUnsavedQuoteChanges ? theme.colors.primary : theme.colors.surface,
                borderColor: theme.colors.border,
              }
            ]}
            onPress={handleSaveQuote}
            disabled={isSaving || !hasUnsavedQuoteChanges}
          >
            <Save size={16} color={hasUnsavedQuoteChanges ? "#ffffff" : theme.colors.textSecondary} />
          </TouchableOpacity>
        )}
      </View>
      ...
      <View style={[styles.tabContainer, { backgroundColor: theme.colors.surface, borderBottomColor: theme.colors.border }]}>
        ...
        <TouchableOpacity
          style={[
            styles.tab,
            activeTab === 'pre-meeting' && styles.activeTab,
            { borderBottomColor: theme.colors.primary }
          ]}
          onPress={() => setActiveTab('pre-meeting')}
        >
          <BookOpen size={16} color={activeTab === 'pre-meeting' ? theme.colors.primary : theme.colors.textSecondary} />
          <Text style={[
            styles.tabText,
            { color: activeTab === 'pre-meeting' ? theme.colors.primary : theme.colors.textSecondary }
          ]} maxFontSizeMultiplier={1.3}>
            Pre meeting
          </Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity
          style={[
            styles.tab,
            activeTab === 'live-meeting' && styles.activeTab,
            { borderBottomColor: theme.colors.primary }
          ]}
          onPress={() => setActiveTab('live-meeting')}
        >
          <ThumbsUp size={16} color={activeTab === 'live-meeting' ? theme.colors.primary : theme.colors.textSecondary} />
          <Text style={[
            styles.tabText,
            { color: activeTab === 'live-meeting' ? theme.colors.primary : theme.colors.textSecondary }
          ]} maxFontSizeMultiplier={1.3}>
            Live meeting
          </Text>
        </TouchableOpacity>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={[styles.notesSection, { backgroundColor: theme.colors.surface }]}>
            <View style={styles.notesHeader}>
              ...
              <View style={styles.notesTitleContainer}>
                ...
                <Text style={[styles.notesSubtitle, { color: theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>
                  Create and manage the word of the day for this meeting
                </Text>
              </View>
            </View>
            ...
            <View style={styles.notesInputSection}>
              ...
              <View style={styles.notesInputHeader}>
                ...
                <Text style={[styles.wordCount, { color: theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>
                  {partOfSpeech.length}/15 characters
                </Text>
              </View>
              ...
              <TextInput
                ...
                placeholderTextColor={theme.colors.textSecondary}
                value={partOfSpeech}
                onChangeText={handlePartOfSpeechChange}
                maxLength={15}
              />
              ...
              <View style={styles.notesInputHeader}>
                ...
                <Text style={[styles.wordCount, { color: theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>
                  {meaning.length}/1000 characters
                </Text>
              </View>
              ...
              <Text style={[styles.notesHint, { color: theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>
                💡 Choose a word that will challenge and inspire your fellow members. Include a clear definition and an example sentence to help everyone understand and use it.
              </Text>
            </View>

            {hasUnsavedWordChanges && (
            ...
            {wordOfTheDay && (
              <View style={[styles.lastSavedInfo, { backgroundColor: theme.colors.background }]}>
                <Text style={[styles.lastSavedText, { color: theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>
                  Last saved: {new Date(wordOfTheDay.updated_at).toLocaleString()}
                </Text>
              </View>
            )}
          </View>
          ...
          <View style={[styles.notesSection, { backgroundColor: theme.colors.surface }]}>
            <View style={styles.notesHeader}>
              ...
              <View style={styles.notesTitleContainer}>
                ...
                <Text style={[styles.notesSubtitle, { color: theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>
                  Create and manage the idiom of the day for this meeting
                </Text>
              </View>
            </View>
            ...
            <View style={styles.notesInputSection}>
              ...
              <TextInput
                ...
                placeholder="Enter the idiom of the day..."
                placeholderTextColor={theme.colors.textSecondary}
                value={idiom}
                onChangeText={handleIdiomChange}
                maxLength={1000}
              />
              ...
              <View style={styles.notesInputHeader}>
                ...
                <Text style={[styles.wordCount, { color: theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>
                  {idiomMeaning.length}/1000 characters
                </Text>
              </View>
              ...
              <TextInput
                ...
                placeholderTextColor={theme.colors.textSecondary}
                value={idiomMeaning}
                onChangeText={handleIdiomMeaningChange}
                multiline
                numberOfLines={3}
                textAlignVertical="top"
                maxLength={1000}
              ...
              <View style={styles.notesInputHeader}>
                ...
                <Text style={[styles.wordCount, { color: theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>
                  {idiomUsage.length}/1000 characters
                </Text>
              </View>
              ...
              <TextInput
                ...
                placeholder="Enter an example sentence using the idiom..."
                placeholderTextColor={theme.colors.textSecondary}
                value={idiomUsage}
                onChangeText={handleIdiomUsageChange}
                multiline
                numberOfLines={3}
                textAlignVertical="top"
                maxLength={1000}
              ...
              <Text style={[styles.notesHint, { color: theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>
                💡 Choose an idiom that will enrich vocabulary and inspire your fellow members. Include a clear definition and an example sentence.
              </Text>
            </View>

            {hasUnsavedIdiomChanges && (
              <View style={[styles.unsavedChangesNotice, { backgroundColor: '#fef3c7' }]}>
                <Text style={[styles.unsavedChangesText, { color: '#f59e0b' }]} maxFontSizeMultiplier={1.3}>
                  You have unsaved changes
                </Text>
            ...
            {idiomOfTheDay && (
              <View style={[styles.lastSavedInfo, { backgroundColor: theme.colors.background }]}>
                <Text style={[styles.lastSavedText, { color: theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>
                  Last saved: {new Date(idiomOfTheDay.updated_at).toLocaleString()}
                </Text>
              </View>
            )}
          </View>
        ...
        {activeTab === 'quote' && (
          <View style={[styles.notesSection, { backgroundColor: theme.colors.surface }]}>
            <View style={styles.notesHeader}>
              <View style={[styles.notesIcon, { backgroundColor: '#8b5cf6' + '20' }]}>
                <MessageSquareQuote size={14} color="#8b5cf6" />
              </View>
              <View style={styles.notesTitleContainer}>
                <Text style={[styles.notesTitle, { color: theme.colors.text }]} maxFontSizeMultiplier={1.3}>
                  Quote of the Day
                ...
                <Text style={[styles.notesSubtitle, { color: theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>
                  Create and manage the quote of the day for this meeting
                </Text>
              </View>
            </View>
            ...
            <View style={styles.notesInputSection}>
              ...
              <TextInput
                ...
                placeholder="Enter the quote of the day..."
                placeholderTextColor={theme.colors.textSecondary}
                value={quote}
              ...
              <View style={styles.notesInputHeader}>
                ...
                <Text style={[styles.wordCount, { color: theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>
                  {quoteMeaning.length}/1000 characters
                </Text>
              </View>
              ...
              <TextInput
                ...
                placeholder="Enter the context or meaning of the quote..."
                placeholderTextColor={theme.colors.textSecondary}
                value={quoteMeaning}
                onChangeText={handleQuoteMeaningChange}
                multiline
                numberOfLines={3}
                textAlignVertical="top"
                maxLength={1000}
              ...
              <Text style={[styles.notesHint, { color: theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>
                💡 Choose a quote that will inspire and motivate your fellow members. Include the meaning or context to help them understand its significance.
              </Text>
            </View>
            ...
            {quoteOfTheDay && (
              <View style={[styles.lastSavedInfo, { backgroundColor: theme.colors.background }]}>
                <Text style={[styles.lastSavedText, { color: theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>
                  Last saved: {new Date(quoteOfTheDay.updated_at).toLocaleString()}
                </Text>
              </View>
            )}
          </View>
        ...
        {activeTab === 'pre-meeting' && (
          <View style={styles.preMeetingContainer}>
            <View style={[styles.subTabBar, { backgroundColor: theme.colors.background }]}>
              <TouchableOpacity
                style={[
                  styles.subTab,
                  preMeetingSubTab === 'word' && styles.activeSubTab,
                  { borderBottomColor: preMeetingSubTab === 'word' ? theme.colors.primary : 'transparent' }
                ]}
                onPress={() => setPreMeetingSubTab('word')}
              >
                <BookOpen size={20} color={preMeetingSubTab === 'word' ? theme.colors.primary : theme.colors.textSecondary} />
                <Text
                  style={[
                    styles.subTabText,
                    { color: preMeetingSubTab === 'word' ? theme.colors.primary : theme.colors.textSecondary }
                  ]}
                  maxFontSizeMultiplier={1.3}
                >
                  Word
              ...
              <TouchableOpacity
                style={[
                  styles.subTab,
                  preMeetingSubTab === 'idiom' && styles.activeSubTab,
                  { borderBottomColor: preMeetingSubTab === 'idiom' ? theme.colors.primary : 'transparent' }
                ]}
                onPress={() => setPreMeetingSubTab('idiom')}
              >
                <Lightbulb size={20} color={preMeetingSubTab === 'idiom' ? theme.colors.primary : theme.colors.textSecondary} />
                <Text
                  style={[
                    styles.subTabText,
                    { color: preMeetingSubTab === 'idiom' ? theme.colors.primary : theme.colors.textSecondary }
                  ]}
                  maxFontSizeMultiplier={1.3}
                >
                  Idiom
              ...
              <TouchableOpacity
                style={[
                  styles.subTab,
                  preMeetingSubTab === 'quote' && styles.activeSubTab,
                  { borderBottomColor: preMeetingSubTab === 'quote' ? theme.colors.primary : 'transparent' }
                ]}
                onPress={() => setPreMeetingSubTab('quote')}
              >
                <MessageSquareQuote size={20} color={preMeetingSubTab === 'quote' ? theme.colors.primary : theme.colors.textSecondary} />
                <Text
                  style={[
                    styles.subTabText,
                    { color: preMeetingSubTab === 'quote' ? theme.colors.primary : theme.colors.textSecondary }
                  ]}
                  maxFontSizeMultiplier={1.3}
                >
                  Quote
            ...
            {preMeetingSubTab === 'word' && (
              <View style={styles.formContainer}>
                {!isGrammarianOfDay() && (
                  <View style={[styles.accessDeniedBanner, { backgroundColor: '#fef3c7' }]}>
                    <AlertCircle size={16} color="#f59e0b" />
                    <Text style={[styles.accessDeniedBannerText, { color: '#92400e' }]} maxFontSizeMultiplier={1.3}>
                      Only the assigned Grammarian can edit
                    </Text>
                  </View>
                )}
                ...
                <View style={[styles.fieldCard, { backgroundColor: theme.colors.surface }]}>
                  <View style={styles.fieldHeader}>
                    ...
                    <Text style={[styles.charCount, { color: word.length >= 100 ? '#ef4444' : theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>
                      {word.length}/100
                    </Text>
                  </View>
                  <TextInput
                ...
                <View style={[styles.fieldCard, { backgroundColor: theme.colors.surface }]}>
                  <View style={styles.fieldHeader}>
                    ...
                    <Text style={[styles.charCount, { color: partOfSpeech.length >= 50 ? '#ef4444' : theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>
                ...
                <View style={[styles.fieldCard, { backgroundColor: theme.colors.surface }]}>
                  <View style={styles.fieldHeader}>
                    ...
                    <Text style={[styles.charCount, { color: meaning.length >= 500 ? '#ef4444' : theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>
                      {meaning.length}/500
                    </Text>
                  </View>
                  <TextInput
                ...
                <View style={[styles.fieldCard, { backgroundColor: theme.colors.surface }]}>
                  <View style={styles.fieldHeader}>
                    ...
                    <Text style={[styles.charCount, { color: usage.length >= 500 ? '#ef4444' : theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>
                      {usage.length}/500
                    </Text>
                  </View>
                  <TextInput
                  ...
                  <View style={styles.actionButtons}>
                    {(word || partOfSpeech || meaning || usage) ? (
                      ...
                      >
                        ...
                        <Text style={[styles.clearButtonText, { color: theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>Clear</Text>
                      </TouchableOpacity>
                    ) : null}
                    <TouchableOpacity
                    ...
                    >
                      ...
                      <Text style={styles.saveDraftButtonText} maxFontSizeMultiplier={1.3}>
                ...
                {wordOfTheDay && (
                  <Text style={[styles.lastSaved, { color: theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>
                    Last saved: {new Date(wordOfTheDay.updated_at).toLocaleString('en-US', { month: '2-digit', day: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true })}
                  </Text>
                )}
              </View>
            )}

            {preMeetingSubTab === 'idiom' && (
              <View style={styles.formContainer}>
                {!isGrammarianOfDay() && (
                  <View style={[styles.accessDeniedBanner, { backgroundColor: '#fef3c7' }]}>
                    <AlertCircle size={16} color="#f59e0b" />
                ...
                <View style={[styles.fieldCard, { backgroundColor: theme.colors.surface }]}>
                  <View style={styles.fieldHeader}>
                    ...
                    <Text style={[styles.charCount, { color: idiom.length >= 200 ? '#ef4444' : theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>
                      {idiom.length}/200
                    </Text>
                  </View>
                  <TextInput
                ...
                <View style={[styles.fieldCard, { backgroundColor: theme.colors.surface }]}>
                  <View style={styles.fieldHeader}>
                    ...
                    <Text style={[styles.charCount, { color: idiomMeaning.length >= 500 ? '#ef4444' : theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>
                      {idiomMeaning.length}/500
                    </Text>
                  </View>
                  <TextInput
                    style={[styles.fieldInputMultiline, { backgroundColor: theme.colors.background, borderColor: theme.colors.border, color: theme.colors.text }]}
                ...
                <View style={[styles.fieldCard, { backgroundColor: theme.colors.surface }]}>
                  <View style={styles.fieldHeader}>
                    ...
                    <Text style={[styles.charCount, { color: idiomUsage.length >= 500 ? '#ef4444' : theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>
                      {idiomUsage.length}/500
                    </Text>
                  </View>
                  <TextInput
                    style={[styles.fieldInputMultiline, { backgroundColor: theme.colors.background, borderColor: theme.colors.border, color: theme.colors.text }]}
                ...
                {idiomOfTheDay && (
                  <Text style={[styles.lastSaved, { color: theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>
                    Last saved: {new Date(idiomOfTheDay.updated_at).toLocaleString('en-US', { month: '2-digit', day: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true })}
                  </Text>
                )}
              </View>
            )}

            {preMeetingSubTab === 'quote' && (
              <View style={styles.formContainer}>
                {!isGrammarianOfDay() && (
                  <View style={[styles.accessDeniedBanner, { backgroundColor: '#fef3c7' }]}>
                    <AlertCircle size={16} color="#f59e0b" />
                ...
                <View style={[styles.fieldCard, { backgroundColor: theme.colors.surface }]}>
                  <View style={styles.fieldHeader}>
                    ...
                    <Text style={[styles.charCount, { color: quote.length >= 150 ? '#ef4444' : theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>
                      {quote.length}/150
                    </Text>
                  </View>
                  <TextInput
                ...
                <View style={[styles.fieldCard, { backgroundColor: theme.colors.surface }]}>
                  <View style={styles.fieldHeader}>
                    ...
                    <Text style={[styles.charCount, { color: quoteMeaning.length >= 400 ? '#ef4444' : theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>
                ...
                {quoteOfTheDay && (
                  <Text style={[styles.lastSaved, { color: theme.colors.textSecondary }]} maxFontSizeMultiplier={1.3}>
                    Last saved: {new Date(quoteOfTheDay.updated_at).toLocaleString('en-US', { month: '2-digit', day: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true })}
                  </Text>
                )}
              </View>
            )}
